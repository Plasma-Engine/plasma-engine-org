...
# Applies standardized label taxonomy across Plasma Engine repositories.
# Requires: gh auth login (scopes: repo).
...
echo -e "${BLUE}Applying Plasma Engine label taxonomy${NC}"
...
for repo in "${REPOS[@]}"; do
  ...
  for entry in "${LABEL_DEFS[@]}"; do
    ...
    if gh label create "${name}" --color "${color}" --description "${description}" -R "${GITHUB_ORG}/${repo}" --force >/dev/null 2>&1; then
      ...
      echo -e "  ${YELLOW}⚠${NC} ${name} (check permissions)"
    fi
  done
done

echo -e "\n${GREEN}Label synchronization complete.${NC}"
